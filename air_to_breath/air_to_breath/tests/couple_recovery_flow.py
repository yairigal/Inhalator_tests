...
def get_all_scenarios(sensors):
    ...
    for state1, state2 in product(states, states):
        scenarios += [state1.params(sensors=[sensor1]), state2.params(sensors=[sensor2])]
...
class CoupleRecoveryFlow2(TestFlow):
    ...
    blocks = [create_flow(name=f"{s1}, {s2} combinations",
                          common={'sensors': [s1, s2]},
                          mode=MODE_OPTIONAL,
                          blocks=[ClearBuffer,
                                  StartSensorSimulation,
                                  ValidateSensors] +
                                  get_all_scenarios([s1, s2]) +
                                  [StopSensorSimulation.params(mode=MODE_FINALLY)])
              for s1,s2 in combinations([setup.pressure, setup.flow, setup.oxygen], 2)]
